...
class start:
    ...
    def format(self, ra=None, dec=None):
        ...
        if starname.split("_")[0] != "Sun":
            ...
            pickle.dump(
                sinfo, open(dir_root + "STAR_INFO/Stellar_info_%s.p" % (starname), "wb")
            )
            query["ins"] = ins
    ...
    def estimate_computation_time(self, fwhm=7.3):
        ...
        rassine_time_required = (
            str(int(rassine_processing // 60))
            + "m"
            + str(int(rassine_processing - 60 * (rassine_processing // 60)))
            + "s"
        )
        snaky_time_required = (
            str(int(snaky_processing // 60))
            + "m"
            + str(int(snaky_processing - 60 * (snaky_processing // 60)))
            + "s"
        )
        ...
        print(
            Fore.CYAN
            + "\n [INFO] RASSINE computation time: %s %s"
            % (rassine_time_required, ["", "(SKIPPED)"][int(self.sy_rassine_db)])
        )
        print(" [INFO] SNAKY computation time: " + snaky_time_required)
        print(
            "\n [INFO] Total computation time estimated: "
            + total_time_required
            + " \n"
            + Fore.RESET
        )
    ...
    def set_dataset(self, starname, ins, files, sub_dico="matching_diff"):
        ...
        if len(files) != 0:
            ...
            if np.prod(check_files) == 0:
                print(
                    Fore.YELLOW
                    + " [EMERGENCY STOP] All the spectra were not found"
                    + Fore.RESET
                )
    ...
    def set_summary(self):
        ...
        if (self.sy_yarara_db == False) & (self.sy_rassine_db == False):
            dace_summary = pd.read_csv(
                dir_root + "DACE_TABLE/Dace_extracted_table.csv", index_col=0
            )
            ...
            summary = pd.DataFrame(
                np.array([files, inss, jdb, berv, flag]).T,
                columns=["filename", "ins", "jdb", "berv", "flag"],
            )
            ...
            if (self.sy_rassine_db == True) & (self.sy_yarara_db == False):
                ...
                summary = pd.read_csv(
                    dir_root + "WORKSPACE/Analyse_summary.csv", index_col=0
                )
    ...
    def copy_yarara(self, yarara_root):
        ...
        os.system(
            "cp "
            + yarara_root
            + "STAR_INFO/Stellar_info_%s.p " % (star)
            + dir_root
            + "STAR_INFO"
        )
        ...
        if "DRS" in sinfo["Ra"].keys():
            ...
            pickle.dump(
                sinfo, open(dir_root + "STAR_INFO/Stellar_info_%s.p" % (star), "wb")
            )
            ra = sinfo["Ra"]["DRS"]
            dec = sinfo["Dec"]["DRS"]
        else:
    ...
    def preprocess(self):
        ...
        if self.sy_rassine_db == False:
            if ins[0:6] == "SOPHIE":
                ...
                (ins.split("_")[0][0:5] == "HARPS")
                | (ins.split("_")[0] == "HARPN")
                | (ins.split("_")[0] == "ESPRESSO")
            ):
                for f in files:
    ...
    def check_spectra(self):
        ...
        if np.sum(summary["flag1"] == 0) == 0:
            print(
                Fore.YELLOW + " [WARNING] No good spectra (Emergency stop)" + Fore.RESET
            )
            print("\n")
    ...
    def compute_rv_sys(self):
        ...
        print(
            "\n [INFO] Final aproximated RV_sys = %.1f +/- %.1f kms"
            % (rv_sys_approx, rv_sys_std)
        )
        ...
        if rv_sys_std > 10:
            print(
                Fore.YELLOW
                + " [WARNING] RV_SYS RMS high (%.1f km/s), SB flag" % (rv_sys_std)
                + Fore.RESET
            )
            sb_flag2 = True
            pd.DataFrame(
                np.array([files, rv_sys]).T, columns=["files", "rv_sys"]
            ).to_csv(dir_root + "WARNING/RV_SYS_JITTER.csv")
            rv_sys_approx = mym.yarara_rough_rv_sys(spec, teff=teff, verbose=self.debug)
        print(
            "\n [INFO] RV_sys initial guess = %.1f +/- %.1f kms"
            % (rv_sys_approx, rv_sys_std)
        )
        sinfo2, sb_flag1 = mym.yarara_check_rv_sys_wrapper(
            dir_root, spec, rv_sys_approx, ccf_tag=""
        )
        ...
        if rcorr < 0.75:
            print(
                Fore.YELLOW
                + " [EMERGENCY STOP] The CCF is unusual. The spectra is unusual."
                + Fore.RESET
            )
            print("\n")
        ...
        if sb_flag1 | sb_flag2:
            print(
                Fore.YELLOW
                + " [WARNING] Spectroscopy binary detected, pipeline not designed for them."
                + Fore.RESET
            )
            print("\n")
    ...
    def compute_ccf(self):
        ...
        if sum(kept) != 0:
            ...
            print(
                " [INFO] Number of good spectra (after CCF check) = %.0f" % (sum(kept2))
            )
            print(
                " [INFO] Number of bad spectra (after CCF check) = %.0f"
                % (len(kept) - sum(kept) + len(kept2) - sum(kept2))
            )
            if np.sum(kept2) == 0:
        ...
        if sum(kept) != 0:
            ...
            if (np.std(ccf_output["rv"].y) > 1000) & (
                np.median(ccf_output["fwhm"].y) < 30
            ):
                sinfo = myf.update_info_lvl2(sinfo, "SB2", "SNAKY", 1)
                print(
                    Fore.YELLOW
                    + " [EMERGENCY STOP] Spectroscopy binary detected"
                    + Fore.RESET
                )
                print("\n")
    ...
    def compute_atmos(self):
        ...
        atmos = mym.yarara_atmos_xgb_spectroscopy(
            dir_root, sinfo, resolution=80000, phot=True
        )
    ...
    def compute_resolution(self):
        ...
        fwhm_ins, berv_output = mym.yarara_instrumental_resolution(
            dir_root, files, np.zeros(len(berv)), berv.copy()
        )
        summary = mym.import_summary(dir_root)  # to reload updated table
        if np.sum(berv != berv_output) != 0:
            summary.loc[
                myf.in1d(np.array(summary["filename"]), files), "berv_computed"
            ] = berv_output
    ...
    def compute_vsini(self, Prot=None, Rs=None):
        ...
        sinfo = mym.import_star_info(dir_root)
        vsini = mym.yarara_vcat(
            dir_root, sub_dico=sub_dico, debug=self.debug, std_bias_kms=0.1
        )
        mym.yarara_vsini(dir_root, Prot=Prot, Rs=Rs)
        sinfo = myf.update_info_lvl2(
            sinfo, "Vsini", "SNAKY", np.round(np.nanmean(vsini), 2)
        )
        pickle.dump(
            sinfo, open(dir_root + "STAR_INFO/Stellar_info_%s.p" % (star), "wb")
        )
    ...
    def compute_activity(self):
        ...
        files = np.array(summary.loc[kept == 1, "filename"])
        ccf_output = mym.import_ccf(dir_root, "G2")
        tab_proxies, CT, mask_activity = mym.yarara_activity_index(
            ccf_output["filename"],
            rv_sys,
            ccf_output["rv"].y,
            material=material,
    ...
    def compute_mhk(self):
        ...
        summary = mym.import_summary(dir_root)
        proxy = np.array(
            summary.loc[myf.in1d(summary["filename"], ccf_output["filename"]), "CaII"]
        )
        dico, rhk, mhk = mym.yarara_activity_mhk(
            dir_root,
            ccf_output["filename"],
            rv_sys,
            ccf_output["rv"].y,
        ...
        summary = pd.merge(
            summary,
            dico[["filename", "RHK", "RHK_std", "MHK", "MHK_std"]],
            on="filename",
            how="left",
        )
        summary.to_csv(dir_root + "WORKSPACE/Analyse_summary.csv")
        ...
        prot_vsini = np.round(
            sinfo["Rstar"]["SNAKY"] * 25 / (sinfo["Vsini"]["SNAKY"] / 2), 1
        )
    ...
    def compute_spectroscopy(self):
        ...
        rv_sys = sinfo["Rv_sys"]["SNAKY"]
        master = myc.tableXY(
            myf.doppler_r(material["wave"], rv_sys * 1000)[1],
            material["reference_spectrum"],
            0 * material["wave"],
        )
        ...
        pickle.dump(
            spectroscopy, open(dir_root + "WORKSPACE/Analyse_spectroscopy.p", "wb")
        )
    ...
    def compute_mag_cycle(self, rm_source=["DACE", "Yu+23"]):
        ...
        try:
            sinfo = mym.import_star_info(dir_root)
            finch_output = mym.yarara_finch(
                dir_root, rm_source=rm_source, offset_instrument="no", ext="_fix_model"
            )
            finch_output = mym.yarara_finch(
                dir_root,
                rm_source=rm_source,
            ...
            pickle.dump(
                ...
                open(
                    dir_root.replace(ins + "/", "ALLINS_MERGED/Pmag_FINCH_info.p"), "wb"
                ),
            )
        except:
            pass
    ...
    def reset(self, suppression="minimal"):
        if self.warning_printed == 1:
            ...
            for j in liste:
                print(" • " + j)
            print(
                "\n [WARNING] If you want to reset, please run .reset() again."
                + Fore.RESET
            )
    ...
    def monitor_ram(self, stage=0):
        ...
        self.memory_history.append(
            [stage, round(current / 1e9, 4), round(peak / 1e9, 4), round(rss, 4)]
        )

        print(
            Fore.CYAN + f"\n [INFO] RAM [Gb] python current/peak: "
            f"{current / 1e9:.4f} / {peak / 1e9:.4f} | "
            f"total process: {rss:.4f}" + Fore.RESET
        )
    ...
    def write_progress(self, stage, step, savefile=None):
        ...
        table_time = pd.DataFrame(
            self.time_step.values(), index=self.time_step.keys(), columns=["time_abs"]
        )
        dt = np.hstack([0, np.diff(table_time["time_abs"])])
        table_time["time_step_min"] = dt
        table_time["frac_time"] = (
            100 * table_time["time_step_min"] / np.sum(table_time["time_step_min"])
        )
        ...
        table_time[
            ["time_step_min", "frac_time", "RAM_gb", "RAM_peak_gb", "RAM_all_gb"]
        ] = np.round(
            table_time[
                ["time_step_min", "frac_time", "RAM_gb", "RAM_peak_gb", "RAM_all_gb"]
            ],
        ...
        if step == "end":
            print(
                "\n",
                table_time[
                    ["time_step_min", "frac_time", "stage", "RAM_peak_gb", "RAM_all_gb"]
                ],
                "\n",
            )
            ...
            tag_duration = (
                str(int(duration // 1)) + "m" + str(int((duration % 1) * 60)) + "s"
            )
            print(
                Fore.CYAN
                + "\n [INFO] Processing achieved in "
                + tag_duration
                + Fore.RESET
            )
            print(
                Fore.CYAN
                + " [INFO] Processing time was estimated initially: "
                + self.sy_time_required_est
                + " \n"
                + Fore.RESET
            )
            ...
            plt.plot(
                np.arange(len(table_time)),
                table_time["time_step_min"].values,
                marker="o",
                color="k",
                alpha=0.9,
            )
            plt.xticks(np.arange(len(table_time)))
            for x, y, t in np.array(
                table_time[["stage", "time_step_min", "frac_time"]][1:]
            ):
            ...
            plt.title(
                "Total time = %s minutes | RAM maximum = %.1f Gb | N files = %.0f"
                % (tag_duration, RAM_max, len(self.sy_files))
            )
            ...
            plt.plot(
                np.arange(len(table_time)),
                table_time["RAM_peak_gb"].values,
                marker="o",
                color="k",
            )
            plt.plot(
                np.arange(len(table_time)),
                table_time["RAM_all_gb"].values,
                marker="o",
                color="gray",
            )
            plt.xticks(
                np.arange(len(table_time)),
                labels=list(table_time.index),
                rotation=60,
                ha="right",
            )
    ...
    def reduce(
        ...
        filename_time = (
            dir_root
            + "/REDUCTION_INFO/Time_informations_reduction_snaky_%s.csv"
            % (timestamp_reduction)
        )
        ...
        if automatic_db:
            ...
            force_resolution = (
                bool(1 - mym.check_force_resolution(dir_root)) & force_resolution
            )
            force_vsini = bool(1 - mym.check_force_vsini(dir_root)) & force_vsini
            force_abs_continuum = (
                bool(1 - mym.check_force_abs_continuum(dir_root)) & force_abs_continuum
            )
            force_activity = (
                bool(1 - mym.check_force_activity(dir_root)) & force_activity
            )
            force_mhk = bool(1 - mym.check_force_mhk(dir_root)) & force_mhk
            force_spectroscopy = (
                bool(1 - mym.check_force_spectroscopy(dir_root)) & force_spectroscopy
            )
            force_magcycle = (
                bool(1 - mym.check_force_magcycle(dir_root)) & force_magcycle
            )
        ...
        try:
            ...
            if teff > 7500:
                force_ccf, force_vsini, force_activity, force_mhk, force_magcycle = [
                    False
                ] * 5
            if teff < 4000:
