...
def print_ram(step=""):
    ...
    print(
        Fore.CYAN + " [COMP] RAM used now (%s) : %.2f Go\n" % (step, mem_usage),
        Fore.RESET + "",
    )
...
def smooth(
    y,
    box_pts,
    shape: int | Literal["rectangular", "gaussian", "savgol"] = "rectangular",
...
def find_nearest_ram_friendly(array1, array2, delta, extra_search=1000):
    ...
    for i in array2:  # to spare RAM memory because of find_nearest algo
        ...
        closest_indice.append(
            cut_left + find_nearest(array1[cut_left:cut_right], i)[0][0]
        )
...
def find_nearest_ndim(array, value, znorm=True):
    ...
    dist = np.sum(
        [abs(array1[j] - array2[j][:, np.newaxis]) for j in np.arange(len(array1))],
        axis=0,
    )
    ...
    distance = np.array(
        [
            np.sum(abs(array1[:, i1] - array2[:, i2]))
            for i1, i2 in zip(idx, np.arange(len(array2.T)))
        ]
    )
...
def flat_clustering(length, cluster_output, extended=0, elevation: NDArray | int = 1):
    vec_init = np.arange(length)
    elevation_vector = (
        np.ones(len(cluster_output)) * elevation
...
def planck_function(wave, Teff):
    y = (
        ...
        * myv.c_lum
        / wave**5
        * (np.exp(myv.h_planck * myv.c_lum / (myv.k_boltz * Teff * wave * 1e-10)) - 1)
        ** -1
    )
    return y
...
def black_body_ratio(T0, teff, wave):
    if type(teff) == np.ndarray:
        planck_factor = (
            np.exp(myv.h_planck * myv.c_lum / (myv.k_boltz * T0 * wave * 1e-10)) - 1
        ) / (
            np.exp(
                myv.h_planck
                * myv.c_lum
                / (myv.k_boltz * teff[:, np.newaxis] * wave * 1e-10)
            )
            - 1
        )
    else:
...
def rm_outliers(array, m=1.5, kind="sigma", axis=0, return_borders=False, Plot=False):
    ...
    sup: None | NDArray = None
    inf: None | NDArray = None
...
def gaus(
    ...
    g: NDArray[np.float64] = np.exp(-((x - x0) ** 2) / (2 * sigma**2))

    if norm:
        return g / np.max(g)

    return np.sqrt(2 * np.pi) * sigma * g
...
def broadGaussFast(x, y, sigma, edgeHandling=None, maxsig=None):
    ...
    if edgeHandling == "firstlast":
        ...
        return np.convolve(y, e, mode="same")[nf:-nf]
    elif edgeHandling is None:
        return np.convolve(y, e, mode="same")
...
def posterior_sin_i_from_samples(
    samples_v,
    samples_vsini,
    Ndraw=200000,
    Npost=20000,
    ...
    vsini_sigma = 0.5 * (
        np.percentile(samples_vsini, 84) - np.percentile(samples_vsini, 16)
    )
    ...
    if weights_sum == 0 or not np.isfinite(weights_sum):
        raise RuntimeError(
            "All weights are zero or non-finite; check inputs or increase Ndraw"
        )
    weights /= weights_sum
    ...
    if plot:
        ...
        # --- sin i posterior ---
        axes[0].hist(sin_i, bins=bins_sini, density=True, histtype="step", linewidth=2)
        ...
        # --- i posterior ---
        axes[1].hist(i_deg, bins=bins_i, density=True, histtype="step", linewidth=2)
...
def observatory(instrument="HARPS"):
    if instrument == "HARPS":
        ...
        obs_loc = EarthLocation(
            lat=32.701388 * u.deg, lon=-109.889166 * u.deg, height=3221
        )
