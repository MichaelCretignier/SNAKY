...
**WARNING: Do not add new files by yourself inside the created `output_directory/`. This directory is a dedicated SNAKY workspace and should contain only files generated by SNAKY.**
...
*The `copy_files=True` option copies the RASSINE files locally into the SNAKY output directory, as if they had been generated by the pipeline. This is not required for the code to run successfully, but `.p` pickle files can occasionally become corrupted, and copying them provides a safer workflow. Note that this doubles the storage requirement by duplicating the files on your system. The final decision is left to the user.*
