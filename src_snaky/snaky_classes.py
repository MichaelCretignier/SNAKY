...
class table(object):
    ...
    def plot(
        ...
        if color is None:
            ...
            if len(table) != len(color):
                color = np.arange(len(table))
                print(
                    "[WARNING] The color vector has not the same size (%.0f) than the table : "
                    % (len(color)),
                    np.shape(table),
                )
    ...
    def fit_unique_base(
        self, base_vec, weight=None, ortho_to=None, offset=False, perm=1, iteration=0
    ):
        ...
        for itrt in range(iteration + 1):
            coeff = np.linalg.lstsq(
                base_vec.T * np.sqrt(weight_x)[:, np.newaxis],
                tab * np.sqrt(weight_x)[:, np.newaxis],
                rcond=None,
            )[0].T
        ...
        if perm > 1:  # bootstrap uncertainties
            noise = np.random.randn(dim1, dim2 * perm) / np.sqrt(
                np.hstack([weight.T] * perm)
            )
...
class tableXY(object):
        ...
        if len(yerr) != 0:
            ...
            if sum(~np.isnan(self.y.astype("float"))):
                self.yerr = np.ones(len(self.x)) * myf.mad(
                    myf.rm_outliers(self.y.astype("float"), m=2, kind="sigma")[1]
                )
    ...
    def copy(self):
        new_table = tableXY(
            self.x.copy(), self.y.copy(), self.xerr.copy(), self.yerr.copy()
        )
    ...
    def clip(self, min=[None, None], max=[None, None], replace=True, invers=False):
        ...
        min2 = np.array(min, dtype=object).copy()
        max2 = np.array(max, dtype=object).copy()
    ...
    def export(
        self, name, format="pickle", columns=["x", "y", "yerr", "qc"], species=None
    ):
        ...
        if format == "pickle":
            myf.pickle_dump(
                pd.DataFrame(
                    np.array([self.x, self.y, self.yerr, self.mask_qc, species]).T,
        ...
        if format == "txt":
            np.genfromtxt(
                np.array(self.x, self.y, self.yerr, self.mask_qc, species), name
            )
    ...
    def plot(
        ...
        if len(np.unique(species)) == 1:
            ...
            colors_species = ["k"] + [
                "C%.0f" % (i) for i in range(1, 1 + len(np.unique(species)))
            ]
        ...
        for num, selection in enumerate(np.unique(species)):
            ...
            if ls != "":
                ...
                plt.errorbar(
                    self.x[mask2][loc][sel][qc],
                    self.y[mask2][loc][sel][qc] + offset,
                    xerr=self.xerr[mask2][loc][sel][qc] * int(xerr),
                    yerr=self.yerr[mask2][loc][sel][qc] * int(yerr),
                    fmt=fmt,
                    color=color,
                    alpha=alpha,
                    capsize=capsize,
    ...
    def fit_line(
        ...
        self.yerr[self.yerr == 0] = [np.min(self.yerr), 0.1][
            int(np.min(self.yerr) == 0)
        ]  # to avoid 0 value

        w = 1 / self.yerr**2
        if offset:
            A = np.array(
                [(self.x - np.mean(self.x) * int(recenter)), np.ones(len(self.x))]
            ).T
        else:
            A = np.array([self.x]).T
        ...
        temp = tableXY(
            ...
            self.y
            - (
                (self.x - np.mean(self.x) * int(recenter)) * self.lin_slope_w
    ...
    def fit_GND(
        ...
        beta_fixed=0,
        norm=True,
        mini=[None, None, 1, 0, 1.8],
        maxi=[0, None, None, None, 5],
    ):
    ...
    def fit_gaussian(
        ...
        try:
            result1 = gmodel.fit(
                self.y[mask], fit_params, 1 / self.yerr[mask] ** 2, x=self.x[mask]
            )
            self.lmfit = result1
            self.params = result1.params
    ...
    def fit_rassine(self, par_R, par_Rmax, par_stretching, tag=""):
        ...
        rassine_main(
            [
                "-s",
                myv.SRC_DIR + f"/temp/spectrum_to_normalise{tag}.csv",
                "-r",
                str(par_R),
                "-R",
                str(par_Rmax),
        ...
        rassine_file = pd.read_pickle(
            myv.SRC_DIR + "/temp/RASSINE_spectrum_to_normalise%s.p" % (tag)
        )
        self.rassine_continuum = tableXY(
            rassine_file["wave"], rassine_file["output"]["continuum_linear"]
        )
        self.rassine_output = rassine_file["output"]
        os.system(
            "rm " + myv.SRC_DIR + "/temp/RASSINE_spectrum_to_normalise%s.p" % (tag)
        )
    ...
    def fit_multi_sb(self):
        ...
        for i in range(K):
            ...
            A2 = np.array([[Gii, sum_p], [sum_p, M]], dtype=float)
        ...
        plt.plot(
            ...
            1 - model_c1,
            color="C2",
            label="RV = %.0f | FWHM = %.0f | CT = %.1f"
            % (best_params[0], best_params[1], amp_c1 * 100),
        )
        ...
        for i in range(K):
            ...
            for j in range(i + 1, K):
                S = np.array([[G[i, i], G[i, j]], [G[j, i], G[j, j]]], dtype=float)
        ...
        plt.plot(
            ...
            color="C1",
            alpha=0.7,
            label="RV = %.0f | FWHM = %.0f | CT = %.1f" % (p1[0], p1[1], amp1 * 100),
        )
        plt.plot(
            self.x,
            1 - comp2,
            color="C0",
            alpha=0.7,
            label="RV = %.0f | FWHM = %.0f | CT = %.1f" % (p2[0], p2[1], amp2 * 100),
        )
    ...
    def ccf(
        ...
        mask = mask[
            (myf.doppler_r(mask[:, 0], 300000)[0] < grid.max())
            & (myf.doppler_r(mask[:, 0], 300000)[1] > grid.min()),
            :,
        ]  # supres line farther than 300kms
        ...
        if not len(mask):
            ...
            grid_min = int(
                myf.find_nearest(grid, myf.doppler_r(mask_min, -100000)[0])[0][0]
            )
            grid_max = int(
                myf.find_nearest(grid, myf.doppler_r(mask_max, 100000)[0])[0][0]
            )
            grid = grid[grid_min:grid_max]

            log_grid = np.linspace(
                np.log10(grid).min(), np.log10(grid).max(), len(grid)
            )
            ...
            if not os.path.exists(static):
                ...
                if save_if_missing:
                    ccf_pipeline = fits.open(
                        MATERIAL_DIR + "/MASK_CCF/" + static.split("/")[-1]
                    )
                    ccf_pipeline[0].data[:, 0] = np.log10(
                        myf.doppler_r(10 ** ccf_pipeline[0].data[:, 0], rv_sys)[0]
                    )
            ...
            log_template = interp1d(
                log_grid_mask, log_mask, kind="linear", bounds_error=False, fill_value=0
            )(log_grid)
            ...
            if fit_gaussian:
                ...
                ccf_profile.fit_gaussian(
                    guess=[-amp, center, width, maxi], Plot=Plot, norm=norm
                )
                try:
                    ccf_profile.fit_GND(
                        guess=[-amp, center, width, maxi, 2],
                        color="g",
                        beta_fixed=0,
                        Plot=Plot,
                        norm=norm,
                ...
                try:
                    ...
                    if (
                        (contrast2 > 0.05)
                        & (contrast2 / contrast1 > 0.10)
                        & (contrast2 < contrast1)
                        & (abs(center2) < 100)
                        & (abs(center1 - center2) > 6)
                    ):
                        ...
                        plt.axvline(
                            ...
                            color="r",
                            ls="-.",
                            label="RV =%.1f | CT = %.1f" % (center2, contrast2 * 100),
                        )
                        self.warning_multipeak = 1
                        print(" [WARNING] Multi peak detected!")
                        print(
                            " [INFO] CT1 = %.1f (RV=%.1f km/s) & CT2 = %.1f (RV=%.1f km/s)"
                            % (contrast1 * 100, contrast2 * 100, center1, center2)
                        )
                        plt.legend(loc=4)
                except:
                    pass
                    ...
                    if ccf_profile.convergence:
                        ...
                        if ccf_profile.params["amp"].stderr is not None:
                            amp_err = 100 * (
                                ...
                                / ccf_profile.params["offset"].value
                            )

                        plt.axvline(
                            x=ccf_profile.params["cen"].value, color="r", alpha=0.3
                        )
                        plt.title(
                            "R = %.2f | Beta = %.2f \n C = %.2f +/- %.2f [%%] \n FWHM = %.2f +/- %.2f [km/s] \n RV = %.2f +/- %.2f [m/s]"
                            % (
