...
def main(argv=None):
    ...
    if len(argv)>0:
        optlist,args =  getopt.getopt(argv,'f:s:o:r:R:a:w:l:p:P:e:S:F:')
        for j in optlist:
            ...
            if j[0] == '-l': #anchor file
                anchor_file = j[1]
            if j[0] == '-F': #Radius minimum
                par_fwhm = j[1]
                par_fwhm = float(par_fwhm)
    ...
    if spectrum_name.split('.')[-1]=='fits': # to load a fits file
        ...
        if spectrum_name.split('.')[-1]=='csv':
            ...
            if np.shape(data)[1]!=2:
                logger.warning('Your txt file does not contain two columns, please take care to format correctly your input')
            if (~(data[0][0]==data[0][0]))&((data[1][0]==data[1][0])):
                logger.warning('Your txt file is suspected to contain a header, the first line was removed (please take care to format correctly your txt input)')
                data = data[1:,:]
    ...
    if output_dir!='':
        ...
        if not os.path.isdir(output_dir):
            logger.debug('The directory does not exist yet, creation of the directory')
    ...
    if type(par_stretching)!=str:
        if par_stretching<0:
            logger.warning('par_stretching is smaller than 0, please enter a higher value')
            logger.warning('par_stretching value fixed at 3')
            par_stretching = 3.0
    else:
        if (float(par_stretching.split('_')[1])>1)|(float(par_stretching.split('_')[1])<0):
            logger.warning('par_stretching automatic value should be between 0 and 1')
            logger.warning('par_stretching value fixed at 0.5')
    ...
    if not only_print_end:
        logger.debug('[BEGIN] RASSINE is beginning the reduction')

    begin = time.time()

    if not only_print_end:
        logger.debug('Computation of the local maxima : LOADING' )

    mask_grid = np.arange(len(grid))[(grid-grid)!=0]
    mask_spectre = np.arange(len(grid))[(spectrei-spectrei)!=0]

    if len(mask_grid)>0:
        logger.debug('Nan values were found, replaced by left and right average...')
        for j in mask_grid:
    ...
    if np.sum(np.isnan(grid))|np.sum(np.isnan(spectrei)):
        logger.warning('There is too much NaN values, attempting to clean your data')
    ...
    miny = np.nanpercentile(spectrei,0.001) ; maxy = np.nanpercentile(spectrei,0.999) #wrong and not used

    len_x = maxx - minx
    len_y = np.max(spectrei) - np.min(spectrei)

    wave_5500 = int(ras.find_nearest(grid,5500)[0][0])
    ...
    if not only_print_end:
        logger.debug(f'Spectrum SNR at 5500 : {NRR_0:.0f}')
    ...
    for iteration in range(5): #k-sigma clipping 5 times
        ...
        if not only_print_end:
            logger.debug(f'Number of cosmic peaks removed : {np.sum((spectre>sup_fast)&(spectre>maxi_roll_fast)):.0f}')
    ...
    if  par_fwhm == 'auto':
        ...
        try:
            ...
            popt = np.array([0,-0.5,0.9,15])
            errors_fit = np.array([99.9,99.9,99.9,99.9])

        if not only_print_end:
            logger.debug(f'[AUTO] FWHM computed from the CCF is about : {popt[-1]*2.35:.2f} [km/s]')
    ...
    if (feedback)&(par_smoothing_box != 'auto'):
        ...
        if not only_print_end:
            logger.debug(f'Smoothed kernel length saved at : {ssmoothing.val} with {callback.shape} shape')
        plt.close()
        ...
        if par_smoothing_box == 'auto':
            if par_smoothing_kernel not in ['erf','hat_exp']:
                if not only_print_end:
                    logger.warning('Your smoothing kernel is not correctly specified, pleaser enter either : erf or hat_exp')
                    logger.debug('The kernel is fixed by default to erf kernel')
    ...
    #plt.show()
    if not only_print_end:
        logger.debug(f'Number of cosmic peaks removed : {np.sum(mask):.0f}')
    wave = wave[~mask]
    flux = flux[~mask]
    index = index[~mask]

    #logger.debug('Rough estimation of the typical width of the lines : median=%.3f mean=%.3f'%(np.median(np.diff(wave))/conversion_fwhm_sig,np.mean(np.diff(wave))/conversion_fwhm_sig))
    ...
    if not only_print_end:
        logger.debug(f'Suggestion of a streching parameter to try : {calib_low + (calib_high-calib_low)*0.5:.0f} +/- {(calib_high-calib_low)*0.25:.0f}')

    out_of_calibration = False
    if par_fwhm/conversion_fwhm_sig>30:
        out_of_calibration = True
        logger.warning('Star out of the FWHM calibration range')
    ...
    if type(par_stretching) == str:
        if not out_of_calibration:
            ...
            if not only_print_end:
                logger.debug(f'[AUTO] par_stretching fixed : {par_stretching:.2f}')
        else:
            logger.debug('[AUTO] par_stretching out of the calibration range, value fixed at 7')
            par_stretching = 7
    ...
    if not only_print_end:
        logger.debug('Computation of the local maxima : DONE' )

    loc_max_time = time.time()

    if not only_print_end:
        logger.debug(f'Time of the step : {loc_max_time-begin:.2f}')
    ...
    if not only_print_end:
        logger.debug('Computation of the penality map : LOADING' )
    ...
    if par_R=='auto':
        ...
        if not only_print_end:
            logger.debug(f'[AUTO] R fixed : {par_R:.1f}')
        if par_R > 5:
            if not only_print_end:
                logger.warning('R larger than 5, R fixed at 5')
            par_R = 5
    ...
    if (par_Rmax!=par_R)|(par_Rmax=='auto'):
        ...
        if par_Rmax=='auto':
            ...
            if not (only_print_end)|(threshold<0.2):
                logger.debug('[AUTO] Rmax found around {cluster_length[largest_cluster,5]:.0f} AA and fixed : {par_Rmax:.0f}')
            if par_Rmax > 150:
                if not only_print_end:
                    logger.warning('Rmax larger than 150, Rmax fixed at 150')
                par_Rmax = 150
        ...
        if feedback:
            ...
            if reg.split('_')[0] == 'poly':
                ...
                if not only_print_end:
                    logger.debug('the law should be either poly_d or sigmoid_c_s')

    if not only_print_end:
        logger.debug('Computation of the penality map : DONE' )

    loc_penality_time = time.time()


    if not only_print_end:
        logger.debug(f'Time of the step : {loc_penality_time-loc_max_time:.2f}')
    ...
    if not only_print_end:
        logger.debug('Rolling pin is rolling : LOADING')
    ...
    if not only_print_end:
        logger.debug('Rolling pin is rolling : DONE')

    loc_rolling_time = time.time()

    if not only_print_end:
        logger.debug(f'Time of the step : {loc_rolling_time-loc_penality_time:.2f}')
    ...
    if not only_print_end:
        logger.debug('Edge cutting : LOADING' )
    ...
    if not only_print_end:
        logger.debug('Edge cutting : DONE' )

    loc_cutting_time = time.time()

    if not only_print_end:
        logger.debug(f'Time of the step : {loc_cutting_time-loc_rolling_time:.2f}')
    ...
    if not only_print_end:
        logger.debug('Outliers removing : LOADING')
    ...
    if not only_print_end:
        logger.debug('Outliers removing : DONE')

    loc_outliers_time = time.time()

    if not only_print_end:
        logger.debug(f'Time of the step : {loc_outliers_time-loc_cutting_time:.2f}')
    ...
    if not only_print_end:
        logger.debug(f'Number of points removed to build a more equidistant grid : {len(wave_backup)-len(wave)}')
    ...
    if not only_print_end:
        logger.debug(f'[END] RASSINE has finished to compute your continuum in {end-begin:.2f} seconds \n')
    ...
    if not only_print_end:
        ...
        for i,j in zip(name_parameters,parameters.keys()):
            logger.debug(i+' : '+str(parameters[j]))
        logger.debug('----------------- ')
    ...
    logger.debug(f'Output file saved under : {output_dir+'RASSINE_'+new_file+'.p'} (SNR at 5500 : {output['parameters']['SNR_5500']:.0f})')
